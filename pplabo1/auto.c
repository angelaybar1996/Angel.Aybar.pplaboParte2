 ...
 * \return Retorna 0 (EXITO) -1 (ERROR)
 */
int menuModificarAuto(eAuto autos[],int tamAuto,eMarca marcas[],int tamMar,eColor colores[],int tamCol, int indice)
{
    int retorno=-1;
    int opcion;
    eAuto auxAutos;
 ...
 * \return Retorna 0 (EXITO) -1 (ERROR)
 */
int cargarDescripcionAuto(int id,eAuto autos[],int tamAuto,char desc[])
{
     int retorno=-1;

    if(autos!=NULL && tamAuto>=0 && id>=80000 && id<=80009 && desc!=NULL)
    {
        for(int i=0;i<tamAuto;i++)
        {
            if(autos[i].id==id)
